...
export class LinksModuleService {
  ...
  async createLink(data: CreateLinkDTO): Promise<any> {
    ...
    return this.prisma.link.create({
      data: {
        ...
        accessCount: 0,
        clicksPerUrl: new Array(data.alternativeUrls.length).fill(0),
      },
    });
  }
  ...
  async rotateLink(originalUrl: string): Promise<string> {
    ...
    await this.prisma.link.update({
      ...
      data: {
        ...
        clicksPerUrl: link.clicksPerUrl.map((count, i) =>
  ...
  async getMetrics(originalUrl: string) {
    const link = await this.prisma.link.findUnique({ where: { originalUrl } });
    ...
    return {
      ...
      stats: link.alternativeUrls.map((url, i) => ({
        url,
        clicks: link.clicksPerUrl[i] || 0,
      })),
    };
  }
}
